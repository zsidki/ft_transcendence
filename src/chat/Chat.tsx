...
const Homep = (props: any) => {
    ...
    return (
    <>
        <ChatTopBar value={props.value<0?"0":props.value} setStatus={props?.setStatus} />
        <div   className="Bars"  >
...
const Chat = (props:any) => {
    ...
    const [status, setStatus] = useState<string>(); // 0: normal, 1: livechat , 2: members, 3: add_room
    ...
    const [otheruser, setoUser] = useState<User[]>([]); 
    ...
    const getroom_name = useCallback(() => {
        const tmp = sessionStorage.getItem("room_name");
        if (tmp)
    ...
    useEffect(()=> {
        ...
        //
        //             if (_user)
        //             setoUser(tmproom.users.find((m:any)=>(m.user_name !== JSON.parse(_user).user_name)))}
        //     }
        // })
